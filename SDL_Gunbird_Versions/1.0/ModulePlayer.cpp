...
bool ModulePlayer::Start()
{
	...
	kyoko_powerup = App->audio->LoadWAV("assets/audio/sound/Kyoko_PowerUp.wav");
	...
	kyoko_powerup_limit = App->audio->LoadWAV("assets/audio/sound/Kyoko_LimitPowerUp.wav");
	...
	kyoko_dead = App->audio->LoadWAV("assets/audio/sound/Kyoko_Dead.wav");
...
update_status ModulePlayer::Update()
{
	...
	if (App->input->activateDebug == true) {
		...
		if (App->input->keyboard[SDL_SCANCODE_F4] == KEY_STATE::KEY_DOWN) { //DIRECT WIN
				if (gateF4 == true && deathwin == false && App->render->camera.y >= -21 * SCREEN_SIZE) {
				...
				else if (gateF4 == true && deathwin == false && App->render->camera.y < -21 * SCREEN_SIZE) {
