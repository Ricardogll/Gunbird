...
void ModuleUI::choosePlayer2() 
{
	...
	{
		App->render->Blit(graphics, 164, abs(App->render->camera.y / SCREEN_SIZE), &portraits[order_kyoko[selection_character] - 1], 1.0f);
		App->render->Blit(graphics, 133, abs(App->render->camera.y / SCREEN_SIZE) + 32, &live[order_kyoko[selection_character] - 1], 1.0f);
		if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_DOWN) {
			if (selection_character < 5)
				selection_character += 1;
			if (selection_character == 5)
		...
		if (App->input->keyboard[SDL_SCANCODE_RCTRL] == KEY_STATE::KEY_DOWN) {
			characterP2 = order_kyoko[selection_character];
			selectPlayer2_ui = false;
			assignCharacter = true;
		}
	}
...
void ModuleUI::liveCharacter(uint player) 
{
	...
	if (App->player->characters[player]->type == KYOKO) {
		if (App->player->characters[player]->live >= 1) {
			(App->render->Blit(graphics, 5 + cont, abs(App->render->camera.y / SCREEN_SIZE) + 21, &live[5], 1.0f));
		}
...
void ModuleUI::scene_Trump() {
	if (App->background3->trump_scene == true) {
		if (App->background3->time_scene_trump < 250) {
			App->render->Blit(App->background3->graphics_trump, 0, abs(App->render->camera.y / SCREEN_SIZE - 48), &(App->background3->trump.GetCurrentFrame()));
			for (uint i = 0; i < 2; ++i) {
		...
		if (App->background3->time_scene_trump == 250) {
			for (uint i = 0; i < 2; ++i) {
				if (App->player->characters[i] != nullptr) {
					App->player->characters[i]->desactivateInput = false;
				}
			}
			App->background3->trump_scene = false;
		}
		App->background3->time_scene_trump++;
	}
}