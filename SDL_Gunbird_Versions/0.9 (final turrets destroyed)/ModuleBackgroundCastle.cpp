...
update_status ModuleBackgroundCastle::Update()
{
	...
	if (App->render->camera.y >= (-300 + SCREEN_HEIGHT) * SCREEN_SIZE) {
		for (uint i = 0; i < MAX_ENEMIES; ++i) {
			if (App->enemies->enemies[i] != nullptr) {
				...
				if (App->enemies->enemies[i]->type == ENEMY_TYPES::RCANNON) {
					delete App->enemies->enemies[i];
					App->enemies->enemies[i] = nullptr;
				}
				if (App->enemies->enemies[i]->type == ENEMY_TYPES::ROBOT) {
					delete App->enemies->enemies[i];
					App->enemies->enemies[i] = nullptr;
				}
			}
	...
	if (App->input->keyboard[SDL_SCANCODE_F] == KEY_STATE::KEY_DOWN)
		App->render->camera.y = (-600 + SCREEN_HEIGHT) * SCREEN_SIZE; /////////////////////////////
